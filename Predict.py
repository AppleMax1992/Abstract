...
def load_data(data_src):
    ...
    # data_src = '美国作家托马斯·弗里德曼在畅销书《世界是平的》里描述了全球化进程及其给人们生活带来的巨大改变'
    D = [re.sub('\s|\t|\n', '', data_src)]
    return D
...
if __name__ == '__main__':
    ...
    print(test)
    res =  pd.DataFrame(columns=['title','content','predict_title'])
    for title,content in tzip(test['title'],test['content']):
        # print(content)
        test_data = prepare_data(content,args, tokenizer)
        pred_title = generate(test_data, model, tokenizer, args)
        # print(title,content,pred_title)
        res.loc[len(res.index)] = [title,content,pred_title]
    res.to_csv('result.csv',encoding='utf_8_sig')